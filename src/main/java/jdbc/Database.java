...
//        ResultSet resultSet = connection.createStatement().executeQuery(String.format("SELECT * FROM public.users_emailpin WHERE email='%s'", email));
        ResultSet resultSet = connection.createStatement().executeQuery(String.format("SELECT * FROM public.users_emailpin WHERE deleted_at IS NULL AND email = '%s'", email));
        if(resultSet.next()) {
